...
def plot_a_segment(ax, lon, lat, dat, title='',
    ...
    im = ax.scatter(
        lon, lat, c=dat, s=1, cmap=plt.cm.bwr,
        transform=proj, vmin=vmin, vmax=vmax
    )

    # Only add a colorbar if requested
    if add_cbar:
...
def get_mss22(longitude, latitude):
    ...
    file_path = os.path.join(data_dir, 'mss_sio_32.1_WGS84.nc')
...
class SSH_L2:
    ...
    def load_data(self, fn, s3sys=None, lat_bounds=[], subset_method="default"):
        """
        ...
        if len(lat_bounds) == 2:
            #===============================
            # Added by Suyue Li on 2026-03-23 to handle the special case of the "aviso_3_expert" product, which has a different data structure and requires a different subsetting method.
            #===============================
            if subset_method == "aviso_3_expert":
                dd = self.subset_aviso_3_expert(dd, lat_bounds)
            else:
    ...
    # Added by Suyue Li on 2026-03-23 to handle the special case of the "aviso_3_expert" product, which has a different data structure and requires a different subsetting method.
    # ==============================
    def subset_aviso_3_expert(self, data_in, lat_bounds):
        # along-track "nadir" latitude per line
        lat_line = np.nanmean(data_in["latitude"].data, axis=-1)
        lat_line = np.where(np.isnan(lat_line), 100, lat_line)

        l0, l1 = lat_bounds
        i0 = np.where(np.abs(lat_line - l0) == np.abs(lat_line - l0).min())[0][0]
        i1 = np.where(np.abs(lat_line - l1) == np.abs(lat_line - l1).min())[0][0]
        if i0 > i1:
            i0, i1 = i1, i0
        if i0 == i1:
            return []

        # pick the along-track dimension name (usually num_lines)
        if "num_lines" in data_in.dims:
            line_dim = "num_lines"
        else:
            # fall back: find a dim matching latitude's first dim
            line_dim = data_in["latitude"].dims[0]

        subset_vars = {}
        for name, da in data_in.data_vars.items():
            if line_dim in da.dims:
                subset_vars[name] = da.isel({line_dim: slice(i0, i1)})
            else:
                subset_vars[name] = da  # keep untouched (no along-track dim)

        # keep coordinates too (important)
        subset_coords = {}
        for cname, cda in data_in.coords.items():
            if line_dim in cda.dims:
                subset_coords[cname] = cda.isel({line_dim: slice(i0, i1)})
            else:
                subset_coords[cname] = cda

        return xr.Dataset(subset_vars, coords=subset_coords, attrs=data_in.attrs)
